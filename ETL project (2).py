...
class ETL:
    ...
    def connectdatabase(self):
        try:
            ...
            self.Listbox1.insert(tk.END,"DataBase Connected")
        except:
            self.Listbox1.insert(tk.END,"Unsucessfull Connection")
    ...
    def addcolumn(self):
        try:
            ...
            if(len(self.columsList.curselection())==1):
                ...
                elif(self.vartest.get()==1):
                    s="COUNT("+(",".join(selec)[0:])+")"
                    self.selectedcolumns.append(s)
                elif(self.vartest.get()==2):
                    s="SUM("+(",".join(selec)[0:])+")"
                    self.selectedcolumns.append(s)
                ...
                if(self.Relation.get()=="SPACE"):
                    ...
                    if(self.vartest.get()==3 or self.vartest.get()==4):
                        ...
                        self.selectedcolumns.append("COUNT"+s+" AS "+self.AsEntry.get())    
                    elif(self.vartest.get()==2):
                        self.selectedcolumns.append("SUM"+s+" AS "+self.AsEntry.get())
            ...
            self.Listbox1.insert(tk.END,"ERROR ADDING COLUMNS")     
    ...
    def addconditions(self):
        ...
        self.conditions+=" "+self.Condition1.get()+" "+ self.Field1.get()+" "+self.Relation1.get()+" "+ self.Value1.get()
    ...
    def executequery(self): 
        try:
            ...
            if(self.vartest.get()==1 or self.vartest.get()==2):
                ...
                sourceQueryResult=pd.Series(cursor3)
                targetQueryResult=pd.Series(cursor4)
                
                print('source Table Details\n',sourceQueryResult)
                print('Target Table Details\n',targetQueryResult)
            
                if(sourceQueryResult.equals(targetQueryResult)):
                    print(True)
            ...
            if(self.vartest.get()==3):
                ...
                cursor3.execute(self.sqlsource)
                cursor4.execute(self.sqltarget)
                sourceQueryResult=pd.DataFrame(cursor3)
                targetQueryResult=pd.DataFrame(cursor4)
                print('source Table Details\n',sourceQueryResult)
                print('Target Table Details\n',targetQueryResult)
                print(sourceQueryResult.count())
                print(targetQueryResult.count())
                if(sourceQueryResult.count().equals(targetQueryResult.count())):
                    print(True)
                    self.Listbox1.insert(tk.END,"TRUE")
                else:
                    print(False)
            ...
            if(self.vartest.get()==4):
                cursor3.execute(self.sqlsource)
                sourceQueryResult=pd.Series(cursor3)
                if(sourceQueryResult.empty):
                    print(True)
                    self.Listbox1.insert(tk.END,"NO MisMatch")
                else:
                    print(False)
                    print(sourceQueryResult)
                    self.Listbox1.insert(tk.END,sourceQueryResult.tolist())
        except:
            self.Listbox1.insert(tk.END,"Unsuccessfull Execution Of Query")
    ...
    def __init__(self, top=None):
        ...
        self.Conditionframe1.configure(text='''WHERE''')
        ...
        self.Field1.place(relx=0.409, rely=0.327, relheight=0.106, relwidth=0.532
                         , bordermode='ignore')
        self.Field1.configure(takefocus="")
        
        self.Relation1 = ttk.Combobox(self.Conditionframe1)
        self.Relation1.place(relx=0.409, rely=0.49, relheight=0.106
                             , relwidth=0.532, bordermode='ignore')
        self.Relation1.configure(takefocus="")

        self.Value1 = tk.Entry(self.Conditionframe1)
        self.Value1.place(relx=0.409, rely=0.653, height=24, relwidth=0.518
                         , bordermode='ignore')
        ...
        self.FieldLabel1.place(relx=0.045, rely=0.327, height=21, width=37
                              , bordermode='ignore')
        ...
        self.FieldLabel1.configure(text='''Field''')

        self.RelationLabel1 = tk.Label(self.Conditionframe1)
        self.RelationLabel1.place(relx=0.045, rely=0.49, height=21, width=55
                                 , bordermode='ignore')
        ...
        self.RelationLabel1.configure(text='''Relation''')

        self.ValueLabel1 = tk.Label(self.Conditionframe1)
        self.ValueLabel1.place(relx=0.045, rely=0.653, height=21, width=41
                              , bordermode='ignore')
        ...
        self.ValueLabel1.configure(text='''Value''')

        

        self.Condition1 = ttk.Combobox(self.Conditionframe1)
        self.Condition1.place(relx=0.318, rely=0.122, relheight=0.106, relwidth=0.35, bordermode='ignore')
